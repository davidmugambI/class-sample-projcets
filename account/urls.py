...
urlpatterns = [
    ...
    path('reset-password/', PasswordResetView.as_view(template_name ='account/reset_password.html',
                                                       ...
                                                       email_template_name = 'account/reset_password_email.html'),
                                                        name='password_reset'),
    path('reset-password/done/',PasswordChangeDoneView.as_view(template_engine='account/reset_password_done.html'),
         name='password_reset_done'),
    re_path('reset-password/confirm/(?P<uidb64>[0-9A-Za-z]+)-(?P<token>.+)/',
            PasswordResetConfirmView.as_view(template_engine='account:reset_password_confirm.html',
                                             success_url='account:password_reset_complete'),
            name='password_reset_confirm'),
    path('reset-password/complete/', PasswordResetCompleteView.as_view(template_engine='account/reset_\
